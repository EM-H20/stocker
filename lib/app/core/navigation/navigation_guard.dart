...
class NavigationGuard {
  ...
  static void safePush(BuildContext context, String route, {Object? extra}) {
    ...
    if (_navigationHistory.length > maxHistorySize * 0.8) {
      debugPrint(
          '⚠️ [NAVIGATION_GUARD] 네비게이션 스택이 깊어지고 있습니다: ${_navigationHistory.length}');
    }
  ...
  static void safePushReplacement(BuildContext context, String route,
      {Object? extra}) {
    final currentRoute = GoRouterState.of(context).matchedLocation;

    debugPrint(
        '🛡️ [NAVIGATION_GUARD] PUSH_REPLACEMENT 네비게이션 요청: $currentRoute → $route');
  ...
  static void _addToHistory(String route) {
    ...
    debugPrint(
        '📍 [NAVIGATION_GUARD] 현재 히스토리 크기: ${_navigationHistory.length}');
    if (kDebugMode) {
      debugPrint(
          '📚 [NAVIGATION_GUARD] 최근 히스토리: ${_navigationHistory.take(5).toList()}');
    }
  }
  ...
  static bool _hasExcessiveConsecutiveVisits(String route) {
    ...
    final recentRoutes =
        _navigationHistory.reversed.take(maxConsecutiveSameRoute);
  ...
  static void printNavigationReport() {
    ...
    debugPrint(
        '📊 [NAVIGATION_GUARD] 최근 10개 경로: ${_navigationHistory.reversed.take(10).toList()}');
    ...
    for (int i = 0; i < sortedRoutes.length && i < 5; i++) {
      ...
      debugPrint(
          '📊 [NAVIGATION_GUARD]   ${i + 1}. ${entry.key}: ${entry.value}회');
    }
